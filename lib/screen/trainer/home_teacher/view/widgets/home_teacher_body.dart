                      ...
                      Column(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              CustomCard(img: AppImages.totalRequests,text1:LocaleKeys.totalRequests.tr() ,text2: "${cubit.studentHomeModel!.data!.reservationsCount.toString()}${LocaleKeys.request.tr()}",),
                              CustomCard(img: AppImages.totalCourses,text1:LocaleKeys.coursesNumbers.tr() ,text2: "${cubit.studentHomeModel!.data!.courses.toString()}${LocaleKeys.courses.tr()}",),
                            ],
                          ),
                          Padding(
                            padding:  EdgeInsets.symmetric(vertical: width*0.03),
                            child: Text(LocaleKeys.shareAndTeach.tr(),style:Styles.textStyle16.copyWith(color: AppColors.blackColor2,fontFamily: AppFonts.almaraiBold) ,),
                          ),
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              CustomCard2(img: AppImages.live,text1:LocaleKeys.startLive.tr() ,text2: LocaleKeys.startLiveWithStudents.tr(),textBtn: LocaleKeys.startRecording.tr(),onTap: (){
                                navigateTo(widget: const StartLiveScreen());
                              },),
                              CustomCard2(img: AppImages.uploadFile,text1:LocaleKeys.uploadEducationalFile.tr() ,text2: LocaleKeys.addPdf.tr(),textBtn: LocaleKeys.downloadFile.tr(),onTap: (){
                          ...
                          if(CacheHelper.getData(key: AppCached.isApple)==false)...[
                            Padding(
                              padding:  EdgeInsets.symmetric(vertical: width*0.03),
                              child: Text(LocaleKeys.bookingRequests.tr(),style:Styles.textStyle16.copyWith(color: AppColors.blackColor2,fontFamily: AppFonts.almaraiBold) ,),
                            ),
                            cubit.studentHomeModel!.data!.reservations!.isEmpty?
                            Center(child: Text(LocaleKeys.noReservations.tr(),style:Styles.textStyle16.copyWith(color: AppColors.mainColorText,fontFamily: AppFonts.almaraiBold))):
                            SizedBox(
                              height:context.locale.languageCode=="ar"? height*0.245:height*0.24,
                              child: ListView.separated(itemBuilder: (context, index){
                                return  ListItem(id: cubit.studentHomeModel!.data!.reservations![index].id!,
