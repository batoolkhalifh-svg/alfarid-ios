    ...
    return BlocBuilder<HomeCubit, HomeStates>(builder: (context, state) {
      ...
      controller.addListener(() {
        if(controller.position.maxScrollExtent==controller.offset){
          cubit.currentPage==cubit.coursesModel!.data!.paginate!.totalPages ?
          null: cubit.nextCourses();
        }
      });
      ...
      return SliverToBoxAdapter(
        child: Column(
          children: [
            CustomRow(title: cubit.titles[0], onTap: () {
              Navigator.push(context, MaterialPageRoute(builder: (ctx) =>
                  BlocProvider.value(value: context.read<HomeCubit>(),
                      child: const ImportantCoursesScreen())));
              },),
            const CustomTitleCourseList(),
            SizedBox(
              height: height*.25,
              child:state is LoadingHomeState2? const Center(child: CustomLoading()):
              cubit.coursesModel!.data!.items!.isEmpty?  Image.asset(AppImages.emptyCourses,width: width*0.4):
              ListView.separated(
                ...
                return  ImportantListItem(index: index,);
              }, separatorBuilder: (BuildContext context, int index) {
                    return SizedBox(width: width*0.02,);
              },),
            ),
            CustomRow(title: cubit.titles[1], onTap: () {
              Navigator.push(context, MaterialPageRoute(builder: (ctx) =>
                  BlocProvider.value(value: context.read<HomeCubit>(),
                      child: const ExcellentTeacherScreen())));
             },),
            SizedBox(
              height: height*.25,
              child: state is LoadingHomeState2? const Center(child: CustomLoading()):
              cubit.teacherModel!.data!.items!.isEmpty?  Image.asset(AppImages.emptyCourses,width: width*0.4):
              ListView.separated(
                ...
                scrollDirection: Axis.horizontal,
                itemBuilder: (context,index){
                  return  ExcellentTeacherItem(index: index,);
                }, separatorBuilder: (BuildContext context, int index) {
                return SizedBox(width: width*0.02,);
              },),
            ),
