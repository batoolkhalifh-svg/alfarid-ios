  ...
  Widget build(BuildContext context) {
    return Scaffold(
        body: BlocProvider<CartCubit>(
            ...
            child:
                BlocBuilder<CartCubit, BaseStates>(builder: (context, state) {
              var cubit = CartCubit.get(context);
              return SafeArea(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                    CustomArrow(text: LocaleKeys.cart.tr(),),
                    state is BaseStatesLoadingState? Padding(
                      ...
                      child: const CustomLoading(fullScreen: true,),
                        ...
                        state is BaseStatesErrorState ? Padding(
                          ...
                          child: CustomError(title: state.msg, onPressed: (){cubit.getMyCart();}),
                        ):
                        cubit.myMyCartModel!.data==null || cubit.myMyCartModel!.data!.items!.isEmpty?Padding(
                          padding:  EdgeInsets.only(top: width*0.1),
                      ...
                      child: SingleChildScrollView(
                        child: Padding(
                            ...
                            child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  CustomTextField(ctrl: cubit.discountCtrl,prefixImg: AppImages.discount,isPrefixImg: true,
                                    ...
                                    suffixIcon: InkWell(
                                        ...
                                        child: Padding(
                                          ...
                                          child:state is BaseStatesLoadingState2? const CustomLoading(): Text(LocaleKeys.apply.tr(),style: Styles.textStyle16.copyWith(fontSize: AppFonts.t14),),
                                        )),
                                  ),
                                ...
                                height: height*0.67,
                                   child: ListView.separated(
                                        ...
                                        itemBuilder: (context, index){
                                          return  CartItem(name: cubit.myMyCartModel!.data!.items![index].name.toString(),
                                            img: cubit.myMyCartModel!.data!.items![index].image.toString(),
                                            price: cubit.myMyCartModel!.data!.items![index].price.toString(),
                                            onTapDelete: (){cubit.deleteCart(itemId:cubit.myMyCartModel!.data!.items![index].id!);},
                                          );
                                  ...
                                  CustomButton(text:LocaleKeys.buyBooks.tr(), onPressed: (){
                                    CacheHelper.getData(key: AppCached.isApple)==true? cubit.buyBookFromCart():
                                    navigateTo(widget: PaymentScreen(price: cubit.myMyCartModel!.data!.totalAfterDiscount!,));
