  ...
  Widget build(BuildContext context) {
    return Scaffold(
        body: BlocProvider<LieutenantCubit>(
            ...
            child:
                BlocBuilder<LieutenantCubit, BaseStates>(builder: (context, state) {
              var cubit = LieutenantCubit.get(context);
              return SafeArea(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                    CustomArrow(text: LocaleKeys.lieutenant.tr(),withArrow: false,),
                    Padding(
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  CustomButton(hit: height * 0.062,text: LocaleKeys.myLibrary.tr(), onPressed: (){cubit.allLieutenantModel!.data!.items!.isEmpty?null:cubit.changeExplore(false);}, widthBtn: width*0.41,color: cubit.isExplore==false?true:false),
                                ],
                              ),

                            ])),
                        state is BaseStatesLoadingState ? const CustomLoading(fullScreen: true):
                        state is BaseStatesErrorState? CustomError(title: state.msg, onPressed: (){cubit.fetchLieutenants();}):
                        cubit.allLieutenantModel!.data!.items!.isEmpty? Center(child: EmptyList(img: AppImages.emptyBook, text: LocaleKeys.noBooks.tr())):
                        Expanded(
                          ...
                          GridView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              return LieutenantItem(
                                ...
                                onTapCart: (){cubit.addToCart(bookId: cubit.allLieutenantModel!.data!.items![index].id!);},);
                            },
                          ):
                          cubit.myAllLieutenantModel!.data!.items!.isEmpty? Center(child: EmptyList(img: AppImages.emptyBook, text: LocaleKeys.noBooks.tr())):
                          GridView.builder(
                          padding: EdgeInsetsDirectional.symmetric(horizontal: width*0.06),
