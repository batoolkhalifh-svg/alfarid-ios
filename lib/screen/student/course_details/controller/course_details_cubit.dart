...
class CourseDetailsCubit extends Cubit<CourseDetailsStates> {
  ...
  Future<void> getCourseDetails({required int id})async{
   emit(CourseDetailsLoadingState());
    Map<dynamic,dynamic> response =await myDio(endPoint:'${AppConfig.courseDetails}$id',
        dioType: DioType.get);
  ...
  Future<void> subScribe({required int id})async{
    ...
    Map<dynamic,dynamic> response = await myDio(endPoint: AppConfig.subScribeCourse, dioType: DioType.post,
        dioBody: {
          "course_id" :id,
        });
    debugPrint(response.toString());
  ...
  Future<void> finishCourse({required int lessonId})async{
    emit(ChangeTabsState());
    Map<dynamic,dynamic> response = await myDio(endPoint: AppConfig.finishLesson, dioType: DioType.post,
        dioBody: {
          "lesson_id" :lessonId,
        });
    debugPrint(response.toString());
    debugPrint("444444444444444");
    if(response["status"]) {
      // showToast(text: response["message"], state: ToastStates.success);
      emit(CourseDetailsSuccessState());
    }else{
